...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:saa="http://schema.slothsoft.net/amber/amberdata" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common"
    xmlns:func="http://exslt.org/functions" xmlns:str="http://exslt.org/strings" xmlns:php="http://php.net/xsl" xmlns:save="http://schema.slothsoft.net/savegame/editor" extension-element-prefixes="exsl func str php">

    <!-- <xsl:key name="dictionary-option" match="save:savegame/save:dictionary/save:option" use="../@dictionary-id" /> -->
    ...
    <!-- <func:function name="save:splitNames"> <xsl:param name="names"/> <func:result select="str:tokenize($names)"/> </func:function> <func:function name="save:getNode"> <xsl:param name="name"/> <xsl:param name="context" select="."/> <func:result select="$context//*[@name = $name]"/> </func:function> 
        <func:function name="save:getNodeValue"> <xsl:param name="name"/> <xsl:param name="context" select="."/> <func:result select="$context//*[@name = $name]/@value"/> </func:function> <func:function name="save:getNodes"> <xsl:param name="names"/> <xsl:param name="context" select="."/> <func:result select="exsl:node-set(save:getNodesHelper($names, 
        $context))/*"/> </func:function> <func:function name="save:getNodesHelper"> <xsl:param name="names"/> <xsl:param name="context"/> <func:result> <xsl:for-each select="str:tokenize($names)"> <xsl:copy-of select="$context//*[@name = current()]"/> </xsl:for-each> </func:result> </func:function> -->
    ...
    <xsl:template match="save:archive[@name='Party_char.amb']" mode="form-content">
        ...
        <xsl:call-template name="savegame.tabs">
            ...
            <xsl:with-param name="list">
                <xsl:for-each select="save:file">
                    <!-- <script type="application/javascript"><![CDATA[ window.addEventListener( "load", (eve) => { AngularAmber.controller("character]]><xsl:value-of select="@name"/><![CDATA[", function($scope) { $scope.name = "]]><xsl:value-of select="@name"/><![CDATA["; alert($scope.name); }); }, 
                        false ); ]]></script> -->
    ...
    <xsl:template match="save:archive[contains(@name, 'Icon_data.amb') or contains(@name, 'Lab_data.amb')]" mode="form-content">
        <xsl:call-template name="savegame.tabs">
    ...
    <xsl:template match="save:archive[@name='2Map_texts.amb'] | save:archive[@name='3Map_texts.amb']" mode="form-content">
        <xsl:call-template name="savegame.tabs">
            <xsl:with-param name="label" select="'Aktive Karte:'" />
    ...
    <xsl:template name="savegame.amber.events">
        <xsl:for-each select=".//*[@name = 'events']">
            ...
            <xsl:for-each select="save:instruction[@type = 'event-dictionary'][*]">
                <div class="events">
                    <!-- <xsl:call-template name="savegame.flex"> <xsl:with-param name="label" select="'Events'"/> <xsl:with-param name="class" select="'events'"/> <xsl:with-param name="items"> <xsl:for-each select="save:instruction[@type = 'event']"> <div> <h3 class="name"><xsl:value-of select="@name"/></h3> 
                        <xsl:call-template name="savegame.table"> <xsl:with-param name="items"> <xsl:apply-templates select="save:instruction[@type = 'event-step']/*" mode="item"/> </xsl:with-param> </xsl:call-template> </div> </xsl:for-each> </xsl:with-param> </xsl:call-template> -->
                    <h3 class="name">Events</h3>
                    <table>
                        <thead>
                            <tr>
