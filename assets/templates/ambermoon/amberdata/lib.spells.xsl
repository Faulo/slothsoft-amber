<xsl:stylesheet version="1.0" xmlns="http://schema.slothsoft.net/amber/amberdata" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saa="http://schema.slothsoft.net/amber/amberdata" xmlns:sse="http://schema.slothsoft.net/savegame/editor">
    ...
    <xsl:template match="sse:savegame">
        <xsl:variable name="spell-types" select="sse:archive[@type='AM2'][1]//*[@name='spell-types']/*" />
        <xsl:variable name="spell-data" select="sse:archive[@type='AM2'][1]//*[@name='spell-data']/*" />
        <xsl:variable name="spell-names" select="sse:archive[@type='AM2'][1]//*[@name='spell-names']/*" />
    ...
    <xsl:template name="extract-spell">
        ...
        <xsl:if test="string-length($spell-name)">
            <spell id="{$spell-id}" name="{$spell-name}">
                ...
                <xsl:for-each select="$spell-data//*[@name='spell-places']">
                    <xsl:call-template name="extract-spell-places" />
                </xsl:for-each>
            </spell>
        </xsl:if>
    </xsl:template>

    <xsl:template name="extract-spell-places">
        <xsl:param name="root" select="." />
        <xsl:for-each select="$root/*[@value=1]">
            <spell-place name="{saa:getName()}" />
        </xsl:for-each>
    </xsl:template>
</xsl:stylesheet>