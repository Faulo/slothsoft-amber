		...
		<xsl:call-template name="savegame.tabs">
			...
			<xsl:with-param name="list">
				<xsl:for-each select="sse:file">
					<!-- <script type="application/javascript"><![CDATA[ window.addEventListener( "load", (eve) => { AngularAmber.controller("character]]><xsl:value-of select="@name"/><![CDATA[", function($scope) { $scope.name 
						= "]]><xsl:value-of select="@name"/><![CDATA["; alert($scope.name); }); }, false ); ]]></script> -->
