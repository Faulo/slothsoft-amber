...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saa="http://schema.slothsoft.net/amber/amberdata"
	...
	<xsl:import href="farah://slothsoft@amber/templates/ambermoon/editor/globals/maps" />
	...
	<xsl:template match="sse:archive[@name='2Map_texts.amb']" mode="form-content">
		<xsl:call-template name="maps.text-file" />
	</xsl:template>
	...
	<xsl:template match="sse:archive[@name='3Map_texts.amb']" mode="form-content">
		<xsl:call-template name="maps.text-file" />
	</xsl:template>
