...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saa="http://schema.slothsoft.net/amber/amberdata"
	xmlns:sse="http://schema.slothsoft.net/savegame/editor" xmlns:sfx="http://schema.slothsoft.net/farah/xslt" xmlns:exsl="http://exslt.org/common" xmlns:func="http://exslt.org/functions"
	...
	<xsl:template name="savegame.amber.events">
		<xsl:for-each select=".//*[@name = 'events']">
			...
			<xsl:for-each select="sse:instruction[@type = 'event-dictionary'][*]">
				<div class="events">
					<!-- <xsl:call-template name="savegame.flex"> <xsl:with-param name="label" select="'Events'"/> <xsl:with-param name="class" select="'events'"/> <xsl:with-param name="items"> <xsl:for-each select="sse:instruction[@type 
						= 'event']"> <div> <h3 class="name"><xsl:value-of select="@name"/></h3> <xsl:call-template name="savegame.table"> <xsl:with-param name="items"> <xsl:apply-templates select="sse:instruction[@type = 'event-step']/*" 
						mode="item"/> </xsl:with-param> </xsl:call-template> </div> </xsl:for-each> </xsl:with-param> </xsl:call-template> -->
					<h3 class="name">Events</h3>
					<table>
						<thead>
							<tr>
	...
	<xsl:template match="*" mode="portrait-picker">
		<amber-picker infoset="lib.portraits" type="portrait" class="portrait-picker" contextmenu="amber-picker-portrait" role="button" tabindex="0"
	...
	<xsl:template match="*" mode="tile-picker">
		<amber-picker infoset="lib.tileset.icons" type="tileset-icon" class="tile-picker" contextmenu="amber-picker-tileset-icon" role="button" tabindex="0"
			onclick="savegameEditor.openPopup(arguments[0])">
	...
	<xsl:template match="*" mode="monster-sprite-picker">
		<amber-picker infoset="lib.monsters" type="monster-sprite" class="monster-sprite-picker" contextmenu="amber-picker-monster-sprite" role="button" tabindex="0"
			onclick="savegameEditor.openPopup(arguments[0])">
	...
	<xsl:template match="*" mode="form-attributes">
		...
		<!-- <xsl:if test="string-length(@template)"> <xsl:attribute name="data-template"><xsl:value-of select="@template" /></xsl:attribute> </xsl:if> <xsl:if test="string-length(@dict)"> <xsl:attribute name="data-dictionary"><xsl:value-of 
			select="@dict" /></xsl:attribute> </xsl:if> <xsl:if test="string-length(@instruction)"> <xsl:attribute name="data-instruction"><xsl:value-of select="@instruction" /></xsl:attribute> </xsl:if> <xsl:if test="string-length(@name)"> 
			<xsl:attribute name="data-name"><xsl:value-of select="@name" /></xsl:attribute> </xsl:if> -->
	</xsl:template>
