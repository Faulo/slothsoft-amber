...
<xsl:stylesheet version="1.0" 
			...
			<xsl:for-each select="sse:instruction[@type = 'event-dictionary'][*]">
				<div class="events">
					<!-- <xsl:call-template name="savegame.flex"> <xsl:with-param name="label" select="'Events'"/> <xsl:with-param name="class" 
						select="'events'"/> <xsl:with-param name="items"> <xsl:for-each select="sse:instruction[@type = 'event']"> <div> <h3 class="name"><xsl:value-of 
						select="@name"/></h3> <xsl:call-template name="savegame.table"> <xsl:with-param name="items"> <xsl:apply-templates select="sse:instruction[@type 
	...
	<xsl:template
		match="sse:group[*[@name = 'current'] and *[@name = 'current-mod'] and *[@name = 'maximum-mod']]" mode="form-content">
		<xsl:apply-templates select="*[@name = 'current']" mode="form-content" />
		<xsl:text>+</xsl:text>
		<xsl:apply-templates select="*[@name = 'current-mod']" mode="form-content" />
		<xsl:text>/</xsl:text>
	...
	<xsl:template match="sse:string | sse:integer | sse:signed-integer | sse:bit | sse:select"
		mode="form-picker">
				...
				<select class="command" disabled="disabled">
					<xsl:for-each select="$options">
						<option value="{position() - 1}">
							<xsl:if
								test="count(ancestor::sse:file/../sse:file) &gt; 1 and ancestor::sse:file/@file-name != current()">
